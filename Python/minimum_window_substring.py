...
#         # count of total strings needed in window
#         chars_to_match = len(t_dict)

#         for right_pointer in range (len(s)):

#             if s[right_pointer] in t_dict:
#                 #update the value in the dicitonary
#                 t_dict[s[right_pointer]] -= 1
#                 # check if we have found all instances of the char in the window
#                 if t_dict[s[right_pointer]] == 0:
#                     chars_to_match -= 1

#                 # increment left pointer while all chars in the window
#                 while chars_to_match == 0:

#                     # update min len and min_dindow
#                     if right_pointer - left_pointer + 1 < min_len:
#                         min_substring = s[left_pointer:right_pointer+1]
#                         min_len = len(min_substring)
                    
#                     # check if updating the left pointer will remove a character that is in t
#                     if s[left_pointer] in t_dict:
#                         t_dict[s[left_pointer]] += 1
#                         if t_dict[s[left_pointer]] > 0:
#                             chars_to_match += 1
